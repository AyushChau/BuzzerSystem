...
function startTimer() {
  ...
  timeInterval = setInterval(function () {
    if (timer == 0) {
      ...
      $("#Timer").html(
        "<h3> 0" + minutes + ":" + tens_seconds + ones_seconds + "</h3>"
      );
    }
    timer--;
  }, 1000);
...
function refreshConnectionDisplay() {
  ...
  for (var i = 0; i < players.length; i++) {
    if (used_hint.includes(players[i])) {
      content +=
        "<h2><span style='color: green'>" + players[i] + "</span></h2>";
    } else {
...
function enableBuzzer() {
  $("#bstatus").html(
    "<h2 class='enabled'>Buzzers: Enabled</h2>\n<div id='Timer'><h3>00:00</h3></div>"
  );
...
function disableBuzzer() {
  ...
  $("#bstatus").html(
    "<h2 class='disabled'>Buzzers: Disabled</h2>\n<div id='Timer'><h3>00:00</h3></div>"
  );
  socket.emit(
    "message",
    JSON.stringify({
